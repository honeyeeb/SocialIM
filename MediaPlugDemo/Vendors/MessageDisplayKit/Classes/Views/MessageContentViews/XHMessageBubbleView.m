...
//    CGSize sizeWithFont = [text sizeWithFont:systemFont constrainedToSize:textSize lineBreakMode:NSLineBreakByWordWrapping];
    CGSize sizeWithFont = [text boundingRectWithSize:textSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName : systemFont } context:nil].size;
...
+ (CGSize)neededSizeForPhoto:(UIImage *)photo {
    ...
    if (photo.size.width != 0 && photo.size.height != 0) {
        scale = photo.size.width / photo.size.height;
    }
    CGFloat width = 140;
...
//            displayTextView.textColor = [UIColor colorWithWhite:0.143 alpha:1.000];
        ...
        if (!_voiceDurationLabel) {
            UILabel *voiceDurationLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 30, 36, 20)];
