...
@property (nonatomic, strong) XHEmotionCollectionViewFlowLayout *flowLayout;
...
- (void)reloadData {
    ...
    if (emotionManager.emotionType == XHBubbleEmotionTypeEmoji) {
        // one page emoji's count
        int count = MDK_SCREEN_WIDTH / (kXHEmotionEmojiSize + kXHEmotionEmojiMinimumLineSpacing) * 3;
        self.emotionPageControl.numberOfPages = (numberOfEmotions / count + (numberOfEmotions % count ? 1 : 0));
    } else {
...
- (XHEmotionCollectionViewFlowLayout *)flowLayout {
    if (!_flowLayout) {
        _flowLayout = [[XHEmotionCollectionViewFlowLayout alloc] init];
    }
    return _flowLayout;
}
...
- (void)setup {
    ...
    if (!_emotionCollectionView) {
        UICollectionView *emotionCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kXHEmotionPageControlHeight - kXHEmotionSectionBarHeight) collectionViewLayout:self.flowLayout];
