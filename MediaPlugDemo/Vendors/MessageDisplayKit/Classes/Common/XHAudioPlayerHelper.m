...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
             ...
             */
            NSError *playErr = nil;
            AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:fileName] error:&playErr];
            if (playErr) {
                NSLog(@"===play filename :%@===play error :%@", fileName, playErr);
            }
            pl.delegate = self;
            [pl play];
            self.player = pl;
